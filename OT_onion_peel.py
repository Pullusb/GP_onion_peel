...
class GPOP_OT_onion_peel_tranform(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.types.ViewLayer.old_mat = old
        bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='MEDIAN')
        bpy.types.ViewLayer.origin_geo = peel.matrix_world.copy()
        bpy.types.ViewLayer.origin_offset_mat = old.inverted() @ peel.matrix_world
...
class GPOP_OT_onion_back_to_object(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        origin_offset_mat = getattr(vl, 'origin_offset_mat')
        if origin_offset_mat:
            ...
            ## go exactly back to where it was ! ::
            # mat = ob.matrix_world @ vl.origin_geo.inverted() @ vl.old_mat.inverted()

            # nmat = vl.origin_geo @ ob.matrix_world
            print('source_ob.matrix_world: ', source_ob.matrix_world)
            print('source_ob.matrix_world == vl.old_mat: ', source_ob.matrix_world == vl.old_mat)
            print('vl.old_mat: ', vl.old_mat)
            print('vl.origin_geo: ', vl.origin_geo)
            print('ob.matrix_world: ', ob.matrix_world)
            ob.location = ob.location
            print('ob.matrix_world == vl.origin_geo: ', ob.matrix_world == vl.origin_geo)
            
            # mat = nmat.inverted() @ vl.old_mat
            ## not bad when object is at world (0 0 0)
            # mat = ob.matrix_world @ vl.origin_geo.inverted() @ source_ob.matrix_world.inverted()
            
            
            # test again
            # mat = vl.origin_geo.inverted() @ source_ob.matrix_world.inverted() @ ob.matrix_world
            # mat = vl.old_mat.inverted() @ vl.origin_geo.inverted() @ ob.matrix_world
            

            ## not bad in placement (rotation breaks)
            # mat = vl.old_mat.inverted() @ vl.origin_geo
            # mat = ob.matrix_world @ source_ob.matrix_world.inverted() @ mat.inverted()
            

            mat = ob.matrix_world @ vl.origin_geo.inverted() @ source_ob.matrix_world.inverted()
            

            context.scene.cursor.location = mat.translation
            print('mat: ', mat)
